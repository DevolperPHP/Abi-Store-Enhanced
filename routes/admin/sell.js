...
// Route for updating quantity - handles both cashier and non-cashier based on order.isCashier
router.put('/edit/update-qty/:orderId/:productId', async (req, res) => {
  try {
    ...
    const user = await User.findOne({ _id: id });
    const sell = await Sell.findOne({ _id: orderId });
    const product = await Product.findOne({ _id: productId });

    if (!user || (!user.isAdmin && !user.permissions.includes('Sell'))) {
    ...
    // Find the specific product in the order by ID
    const targetProduct = sell.products.find(p => p._id.toString() === productId);
    if (!targetProduct) {
      return res.status(404).send('Product not found in order');
    }

    const oldQty = Number(targetProduct.qty);
    const qtyDifference = newQty - oldQty;

    if (newQty === 0) {
      // Remove specific item from order using atomic operation
      await Sell.updateOne(
        { _id: orderId },
        { $pull: { products: { _id: new mongoose.Types.ObjectId(productId) } } }
      );

      // Check order type and update appropriate storage field
      if (sell.isCashier) {
        // Cashier order - return to store storage (stock)
        const currentStock = product.stock || 0;
        await Product.updateOne(
          { _id: productId },
          {
            $set: { stock: currentStock + Number(oldQty) },
            $pull: {
              score: {
                num: oldQty,
        ...
        // Non-cashier order - return to main storage (qty)
        await Product.updateOne(
          { _id: productId },
          { $set: { qty: Number(product.qty) + Number(oldQty) } }
        );
      }

      // Check if order is now empty and delete if so
      const checkSell = await Sell.findOne({ _id: orderId });
      if (checkSell.products.length === 0) {
        await Sell.deleteOne({ _id: orderId });
        return res.redirect('/sell'); // Redirect to dashboard when order is deleted
      }
    } else {
      // Update quantity using atomic operations
      
      const newTotal = product.sell_price * newQty;
      const updateResult = await Sell.updateOne(
        {
          _id: orderId,
    ...
    // Check order type and update appropriate storage field
    if (sell.isCashier) {
      // Cashier order - update store storage (stock)
      const currentStock = product.stock || 0;
      
      if (qtyDifference > 0) {
        // Increasing quantity - reduce store storage
        if (currentStock < qtyDifference) {
          return res.status(400).send('Insufficient store storage');
        }
        await Product.updateOne(
          { _id: productId },
          {
            $set: { stock: currentStock - Number(qtyDifference) },
            $push: {
              score: {
                num: qtyDifference,
                Date: moment().locale("ar-kw").format("l")
        ...
        // Decreasing quantity - return to store storage
        await Product.updateOne(
          { _id: productId },
          {
            $set: { stock: currentStock + Math.abs(qtyDifference) },
            $pull: {
              score: {
                num: Math.abs(qtyDifference)
              }
            }
          }
        );
      ...
      // Non-cashier order - update main storage (qty)
      if (qtyDifference > 0) {
        // Increasing quantity - reduce main storage
        if (product.qty < qtyDifference) {
          return res.status(400).send('Insufficient main storage');
        }
        await Product.updateOne(
          { _id: productId },
          { $set: { qty: Number(product.qty) - Number(qtyDifference) } }
        );
      } else {
        // Decreasing quantity - return to main storage
        await Product.updateOne(
          { _id: productId },
          { $set: { qty: Number(product.qty) + Math.abs(qtyDifference) } }
        );
      }
    }

    // Update the overall order total (only if order still exists)
    const updatedSell = await Sell.findOne({ _id: orderId });
    if (updatedSell) {
      const orderTotal = updatedSell.products.reduce((sum, item) => {
        return sum + (Number(item.total) || 0);
      }, 0);
      
      await Sell.updateOne(
        { _id: orderId },
        { $set: { total: orderTotal.toString() } }
      );
    }
